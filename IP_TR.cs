...
namespace server
{
    class IP_TR
    {
        ...
        public IP_TR()
        {
            ...
            pc_list_all = ((Pc_container)file_serial.Deserialize(new JsonTextReader(file_string), typeof(Pc_container))).pc_list_all;
        }

        public List<PC> name_to_ip(List<int> eq_id_list, List<string> pc_name_list, List<string> pc_side_list)
        {
            return pc_list_all.Where(x => { return x.is_pc_in_type(eq_id_list, pc_name_list, pc_side_list); }).ToList();
        }
